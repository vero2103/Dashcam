...
if __name__ == "__main__":
    ...
    parser.add_argument("--rtsp_url", type=str, help="URL de la cámara IP")
    ...
    # Usar la URL de la cámara IP si está proporcionada
    if opt.rtsp_url:
        cap = cv2.VideoCapture(opt.rtsp_url)
        out = cv2.VideoWriter('output.mp4', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 10, (320, 240))
    else:
    ...
    while True:
        ret, frame = cap.read()
        if not ret:
            print("No se pudo capturar el flujo de video")
            break
        ...
        cv2.imshow('frame', Convertir_BGR(frame))
        out.write(Convertir_BGR(frame))
